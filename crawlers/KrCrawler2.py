...
def crawl(search, start_date, end_date):
    ...
    with open('result.jsonl', 'w', encoding='utf8') as f:
        f.write('')
    ...
    for i, news_id in df_total.NEWS_ID.items():
        if len(url_list)>=100:
            rs = (print(f'{idx+1 + int((i+1)/100)*100} / {len(df_total.NEWS_ID)} 내용 수집 진행 {u}') or grequests.get(f"https://www.bigkinds.or.kr/news/detailView.do?docId={u}&returnCnt=1", headers=headers, callback=fbc.feedback) for idx, u in enumerate(url_list))
            a = grequests.map(rs)
    ...
    rs = (grequests.get(f"https://www.bigkinds.or.kr/news/detailView.do?docId={u}&returnCnt=1", headers=headers, callback=fbc.feedback) for idx, u in enumerate(url_list))
    a = grequests.map(rs)
    ...
    with open('kr_news_%s.json'%search, 'w', encoding='utf8') as f:
...
class FeedbackCounter:
    ...
    def feedback(self, r, **kwargs):
        self.counter += 1
        print(f'{self.counter} / {self.all}, {r.url}')
        return r